...
class Download_Manager:
    ...
    def create_new_download(self, url, path, paused=True):
        if paused:
            paused = "true"
        else:
            paused = "false"
    ...
    def create_new_multiple_downloads(self, urls, path, paused = True):
        createdDownloadObjects = [
            self.create_new_download(i, path, paused = paused)\
            for i in urls
        ]
